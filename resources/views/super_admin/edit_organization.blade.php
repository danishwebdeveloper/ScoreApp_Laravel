...
@section('page_title', 'Talent Ranker - '.ling('edit').' '.ling('org'))


@section('content')

	<div class="m-heading-1 border-green m-bordered">
		<h5 style="display: inline-block">Talent Ranker - {{ ling('edit').' '.ling('org') }}!</h5>
		<a href="{{ url('admin_panel/organizations') }}" class="btn btn-circle green-meadow pull-right"><i class="fa fa-reply"></i> {{ ling('go_back') }}</a>
	</div>
  @php 
    $cData = get_regions_simp($org_data->country_id);
  @endphp
	<div class="row">
        <div class="col-md-12">
            <div class="portlet light ">
                <div class="portlet-title">
                    <div class="caption">
                        ...
                        <span class="caption-subject font-green bold uppercase">{{ ling('edit').' '.ling('org') }}</span>
                    </div>
                </div>
                ...
                <div class="portlet-body form">
                    <!-- BEGIN FORM-->
                    <form action="{{ url('admin_panel/update_organization') }}" class="form-horizontal" method="post">
                      {{ csrf_field() }}
                        <div class="form-body">
                            <div class="form-group">
                                <label class="col-md-3 control-label">{{ ling('name') }}</label>
                                <div class="col-md-6">
                                    <input type="text" class="form-control" name="name" value="{{ $org_data->org_name }}">
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <div class="col-md-6">
                                   <select name="country_id" class="form-control bs-select" id="selCot" data-size="5">
                                     @if(count(countries()) > 0)
                                      ...
                                      <option value="{{ $country->id }}" {{ ( $country->id == $org_data->country_id ) ? 'selected' : '' }}>{{ $country->name }}</option>
                                      @endforeach
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-6">
                                    <select class="bs-select form-control" name="region_id" id="selReg" name="r_id">
                                        @foreach($cData as $key => $region)
                                          <option value="{{ $region->id }}" {{ ( $region->id == $org_data->region_id ) ? 'selected' : '' }}>{{ $region->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="form-actions">
                            <div class="row">
                                <div class="col-md-offset-3 col-md-4">
                                    <input type="hidden" name="org_id" value="{{ $org_data->id }}">
...
@section('javascript')
  ...
  @if(Session::has('message'))
      <script>
        swal("{{ ling('updated') }}!", "{{ ling('org_updated_succ') }}!", "success");
      </script>
  @endif
@stop